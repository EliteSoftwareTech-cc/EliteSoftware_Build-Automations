...
Write-Host "Building EliteSoftware-VersionBumper (C++)..." -ForegroundColor Cyan

$SourceFile = Join-Path $ScriptDir "main.cpp"
$RcFile = Join-Path $ScriptDir "resource.rc"
$Outx64 = Join-Path (Split-Path -Parent (Split-Path -Parent $ScriptDir)) "BuildOutputx64\EliteBuild_VersionBumper.exe"
$Outx86 = Join-Path (Split-Path -Parent (Split-Path -Parent $ScriptDir)) "BuildOutputx86\EliteBuild_VersionBumper.exe"
