...
Write-Host "Building EliteSoftware-ReadmeGenerator (C++)..." -ForegroundColor Cyan

$SourceFile = Join-Path $ScriptDir "main.cpp"
$RcFile = Join-Path $ScriptDir "resource.rc"
$Outx64 = Join-Path (Split-Path -Parent (Split-Path -Parent $ScriptDir)) "BuildOutputx64\EliteReadmeGenerator.exe"
$Outx86 = Join-Path (Split-Path -Parent (Split-Path -Parent $ScriptDir)) "BuildOutputx86\EliteReadmeGenerator.exe"
